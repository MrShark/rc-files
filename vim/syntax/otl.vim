...
" hi def otlLeadingSpaces ctermbg=lightred guibg=#ffdddd
  ...
  hi def otlTagRef ctermbg=white ctermfg=darkblue cterm=underline guibg=lightred
  hi def otlTagDef ctermbg=white ctermfg=darkblue cterm=bold guibg=lightgreen
  hi def otlTextLeader guifg=darkgrey ctermfg=7
endif
...
if g:otl_bold_headers
  hi def otlTab0 ctermfg=black cterm=bold term=reverse
  hi def otlTab1 ctermfg=darkred cterm=bold term=reverse
  hi def otlTab2 ctermfg=darkblue cterm=bold term=reverse
  hi def otlTab3 ctermfg=darkmagenta cterm=bold term=reverse
  hi def otlTab4 ctermfg=black cterm=bold term=reverse
  hi def otlTab5 ctermfg=darkred cterm=bold term=reverse
  hi def otlTab6 ctermfg=darkblue cterm=bold term=reverse
  hi def otlTab7 ctermfg=darkmagenta cterm=bold term=reverse
  hi def otlTab8 ctermfg=black cterm=bold term=reverse
  hi def otlTab9 ctermfg=darkred cterm=bold term=reverse
else
  hi def otlTab0 ctermfg=1 cterm=NONE
